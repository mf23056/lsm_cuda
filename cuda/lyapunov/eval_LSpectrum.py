...
class SNN:
    ...
    def calculate_lyapunov_spectrum(self, epsilon=1e-6, num_steps=1000):
        ...
        for t in range(num_steps):
            ...
            # ヤコビ行列を計算
            jacobian = self.compute_jacobian(original_state, perturbed_state, epsilon)
            jacobians.append(jacobian)

        # ヤコビ行列からLyapunov Spectrumを計算
        lyapunov_spectrum = self.compute_lyapunov_exponents_from_jacobians(jacobians)
        return lyapunov_spectrum

    def compute_jacobian(self, original_state, perturbed_state, epsilon):
        """ ヤコビ行列を計算するメソッド """
        delta_V = perturbed_state["V"] - original_state["V"]
        delta_spike_state = perturbed_state["spike_state"] - original_state["spike_state"]
        
        # 正方行列としてヤコビ行列を構築
        jacobian = torch.zeros((self.n_total, self.n_total), device=self.device)
        
        # Vとスパイク状態に対する変化を元にヤコビ行列を構築
        for i in range(self.n_total):
            jacobian[i, :] = delta_V / epsilon
        for i in range(self.n_total):
            jacobian[i, :] += delta_spike_state / epsilon
        
        return jacobian

    def compute_lyapunov_exponents_from_jacobians(self, jacobians):
        """ ヤコビ行列からLyapunov Exponentを計算するメソッド """
        exponents = []
        for jacobian in jacobians:
            eigenvalues = torch.linalg.eigvals(jacobian)
            # 固有値の実部がLyapunov Exponentに対応
            lyapunov_exponent = torch.real(eigenvalues).max().item()
...
if __name__ == '__main__':
    ...
    # 摂動ありでのシミュレーション
    lyapunov_spectrum = network.calculate_lyapunov_spectrum(epsilon=1e-6, num_steps=2000)

    # 摂動なしスパイクラスタープロット
    # network.plot_spike_raster(spike_record_no_perturb, title="Spike Raster Plot (No Perturbation)")
    
    # 摂動ありスパイクラスタープロット
    spike_record_with_perturb = network.run_simulation(inputs)
    network.plot_spike_raster(spike_record_with_perturb, title="Spike Raster Plot (With Perturbation)")

    # Lyapunov Exponentのプロット
    network.plot_lyapunov_exponents(lyapunov_spectrum, network.dt)
